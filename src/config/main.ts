...
// 从appData获取配置
function getConfigFromAppData(useAppPathConfig: boolean) {
  let config = {};
  if (useAppPathConfig) {
    try {
...
// 获取项目本地config基础配置
function getLocalBaseConfig(configPath: string) {
  try {
...
// 获取项目本地config配置
function getLocalConfig(configPath: string, env: string) {
